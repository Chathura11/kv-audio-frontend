...
export default function ForgotPasswordPage(){
    ...
    async function handleSendOTP(){
        if(email !=""){
            setSending(true);
            await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/users/forgotPassword/sendOTP/${email}`).then((res)=>{
    ...
    async function handleVerify(){
        if(email != ""){
            setVerifing(true);
            await axios.post(`${import.meta.env.VITE_BACKEND_URL}/api/users/forgotPassword/verifyEmail/${email}`,
                {
                    code:parseInt(otp)
                }
    ...
    async function sendEmail(){
        if(email != ""){
            await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/users/forgotPassword/${email}`).then((res)=>{
                setVerifing(false);
